...
export class MermaidParser {
	...
	public parse() {
		...
		const mermaidObjectTree: MermaidParserObjectPlainTree =
			objectMatches.reduce((masterObject, object) => {
				...
				for (let propertyMatch of propertyMatches) {
					if (propertyMatch[3]) {
						const args = [
							...(propertyMatch[4]?.matchAll(regexMap.methodArgs) || []),
						];
		...
		for (let regexResult of relationMatches) {
			...
			mermaidObjectTree[mappedRelation.primarySideName].relations.push(
				mappedRelation
			);
		}
		...
		const mermaidDocData = graphOrder.reduce(
			(masterObject, key): { [key: string]: MermaidObject } => {
				const object = dependencyGraph.getNodeData(
					key
				) as MermaidParserObjectPlain;
				...
				if (relations.superClasses?.length) {
					relations.foreignDependencies = relations.superClasses
						.map((superClass) => findInheritedDependencies(superClass))
						.flat();
				}
